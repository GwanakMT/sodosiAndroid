    ...
    private fun setOnClickListener() {
        binding.btnNext.setOnClickListener {
            if (viewModel.timer.value > 0) {
                val smsCode = "${binding.etCode1.text}${binding.etCode2.text}${binding.etCode3.text}${binding.etCode4.text}${binding.etCode5.text}${binding.etCode6.text}"
                authManager.signInWithPhoneAuthCredential(smsCode, this@Step3Fragment)
            } else {
                Toast.makeText(context, "인증번호가 발송된지 3분이 지났습니다. 인증번호 재전송 버튼을 눌러주세요", Toast.LENGTH_SHORT).show()
            }
        }
