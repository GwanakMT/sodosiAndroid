...
class SodosiRepositoryImpl @Inject constructor(
    ...
    override suspend fun getHotSodosiList(): Result<List<Sodosi>> {
        return try {
            val result = sodosiApi.getHotSodosiList()
            val sodosiList = result.data.map { sodosiMapper.mapToEntitiy(it) }
            Result.Success(sodosiList)
        } catch (e: Exception) {
            Result.Error(e)
    ...
    override suspend fun getNewSodosiList(): Result<List<Sodosi>> {
        return try {
            val result = sodosiApi.getNewSodosiList()
            val sodosiList = result.data.map { sodosiMapper.mapToEntitiy(it) }
            Result.Success(sodosiList)
        } catch (e: Exception) {
            Result.Error(e)
